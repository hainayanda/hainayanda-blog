...
projectApi.get('/all', (req, resp) => {
    ProjectDao.instance.getAll((err, projRes) => {
        if (projRes == null) {
            resp.status(500).send('Failed to get all projects'); 
        } else {
            const jsonResp: Project[] = [];
...
projectApi.get('/:id', (req, resp) => {
    ...
    ProjectDao.instance.getById(id, (err, projRes) => {
        if (projRes == null) { resp.status(500).send('Failed to get project with id:' + id); } else {
            const jsonResp = Project.parsedFrom(projRes);
            resp.json(jsonResp);
        }
    });
});
